...
def DeleteDuplicate(Path = "Marvellous"):
    ...
    LogDirPath = os.path.join(CurrentDirPath,NewFolder)

    timestamp = time.ctime()
    filename = "MarvellousLog_%s.log"%(timestamp)
    filename = filename.replace(" ","_")
    filename = filename.replace(":","_")
    LogFilePath = os.path.join(LogDirPath,filename)
    ...
    for file_paths in result:
        for file_path in file_paths:
            ...
            if count > 1:
                ...
                fobj.write(f"Deleting file : {file_path}\n")
                os.remove(file_path)
        count = 0
    # print("Total Deleted Files: ",TotalDelete)
    fobj.write(f"Total Deleted Files: {TotalDelete}\n")
...
def main():
    ...
    if len(sys.argv) == 2:
        if(sys.argv[1] == "--h" or sys.argv[1] == "--H"):
            ...
            DeleteDuplicate(Path=sys.argv[1])
    else:
    ...
    print(Border)
    # MaintainLog("This is log message")
